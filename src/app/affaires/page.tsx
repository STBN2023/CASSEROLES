...
export default function AffairesPage() {
  ...
  const [affaireToElus, setAffaireToElus] = useState<Record<string, EluLie[]>>({})
  ...
  useEffect(() => {
    Promise.all([
      fetch("/data/affaires.json").then((r) => r.json()),
      fetch("/data/affaires-elus.json").then((r) => r.json()),
    ]).then(([affairesData, indexData]: [Affaire[], Record<string, EluLie[]>]) => {
      setAffaires(affairesData)
      setAffaireToElus(indexData)
      setLoading(false)
    })
  }, [])
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      r = r.filter((a) => {
        // Recherche dans les champs de l'affaire
        const matchAffaire =
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Base de données Transparency International France et Wikidata · Atteintes à la probité en France.
        </p>
      </div>
      ...
      {loading ? (
        <div className="text-center py-20 text-gray-400">Chargement des affaires…</div>
      ) : (
        <>
            ...
            {paged.map((affaire) => {
              ...
              return (
                <div key={affaire.id} className="bg-white rounded-xl border border-gray-200 p-5">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge variant="outline" className="text-[10px] text-gray-400">
                        {affaire.id.startsWith("wd_") ? "Wikidata" : "TI France"}
                      </Badge>
                    </div>
                  ...
                  {/* Personnes impliquées (TI France) si pas d'élu lié */}
